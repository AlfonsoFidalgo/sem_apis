...
CREATE TEMPORARY TABLE tmp_sem_attribution (
	...
	first_successful_payment_transferred TIMESTAMP,
	attr_category_level_3 VARCHAR(100),
...
INSERT INTO tmp_sem_attribution
   ...
   ram.url_first_visit,
	 ...
	 (CASE
 	  ...
 	  END) as adgroup_id,
		...
		ram.attr_category_level_3,
		ram.creation_platform
  ...
  AND date_trunc('day', ram.first_successful_payment_transferred) > current_date - 10
	...
	DELETE FROM reports.sem_attribution sa WHERE date_trunc('day', sa.first_successful_payment_transferred) > current_date - 10;

	--STEP 5: sem attribution data goes into its own table
	INSERT INTO reports.sem_attribution
		SELECT
					tsa.user_id,
					tsa.date_user_created,
					tsa.first_successful_payment_transferred,
					tsa.url_first_visit,
					tsa.engine,
					tsa.creative_id,
					tsa.campaign_id,
					...
					(CASE
					 WHEN tsa.campaign_id = 0 AND tsa.cntry_first_address = 'AUS' THEN 'AU | Consumer | Universal App Campaign | English'
					 WHEN tsa.campaign_id = 0 AND tsa.cntry_first_address = 'GBR' THEN 'UK | Consumer | Universal App Campaigns | English'
					 WHEN tsa.campaign_id = 0 AND tsa.cntry_first_address = 'BRA' THEN 'BR | Consumer | Universal App Campaign | Portuguese'
					 WHEN tsa.campaign_id = 0 AND tsa.cntry_first_address IN ('USA', 'CAN') THEN 'US | Consumer | Universal App Campaigns | English'
					 WHEN tsa.campaign_id = 0 AND tsa.cntry_first_address IN ('JPN', 'HKG', 'NZL', 'SGP') THEN 'All | Consumer | Universal App Campaign | English'
					 WHEN tsa.campaign_id = 0 AND tsa.cntry_first_address NOT IN ('USA', 'CAN', 'AUS', 'GBR', 'JPN', 'HKG', 'NZL', 'SGP') THEN 'EU | Consumer | Universal App Campaign | English'
					...
					(CASE
						WHEN tsa.campaign_id = 0 THEN
 						 (CASE
 						 		WHEN tsa.cntry_first_address = 'AUS' AND tsa.creation_platform = 'iOS App' THEN 'AU | Consumer | iOS Universal App Campaign | English'
 								WHEN tsa.cntry_first_address = 'AUS' AND tsa.creation_platform = 'Android App' THEN 'AU | Consumer | Android Universal App Campaign | English'
 								WHEN tsa.cntry_first_address = 'GBR' AND tsa.creation_platform = 'iOS App' THEN 'UK | Consumer | iOS Universal App Campaign | English'
 								WHEN tsa.cntry_first_address = 'GBR' AND tsa.creation_platform = 'Android App' THEN 'UK | Consumer | Android Universal App Campaign | English'
								WHEN tsa.cntry_first_address = 'BRA' AND tsa.creation_platform = 'iOS App' THEN 'BR | Consumer | iOS Universal App Campaign | Portuguese'
								WHEN tsa.cntry_first_address = 'BRA' AND tsa.creation_platform = 'Android App' THEN 'BR | Consumer | Android Universal App Campaign | Portuguese'
 								WHEN tsa.cntry_first_address IN ('USA', 'CAN') AND tsa.creation_platform = 'iOS App' THEN 'US | Consumer | iOS Universal App Campaign | English'
 								WHEN tsa.cntry_first_address IN ('USA', 'CAN') AND tsa.creation_platform = 'Android App' THEN 'US | Consumer | Android Universal App Campaign | English'
 								WHEN tsa.cntry_first_address IN ('JPN', 'HKG', 'NZL', 'SGP') AND tsa.creation_platform = 'iOS App' THEN 'All | Consumer | iOS Universal App Campaign | English'
 								WHEN tsa.cntry_first_address IN ('JPN', 'HKG', 'NZL', 'SGP') AND tsa.creation_platform = 'Android App' THEN 'All | Consumer | Android Universal App Campaign | English'
								WHEN tsa.cntry_first_address NOT IN ('USA', 'CAN', 'GBR', 'AUS', 'JPN', 'HKG', 'NZL', 'SGP') AND tsa.creation_platform = 'iOS App' THEN 'EU | Consumer | iOS Universal App Campaign | English'
								WHEN tsa.cntry_first_address NOT IN ('USA', 'CAN', 'GBR', 'AUS', 'JPN', 'HKG', 'NZL', 'SGP') AND tsa.creation_platform = 'Android App' THEN 'EU | Consumer | Android Universal App Campaign | English'
 						 END)
					 WHEN tsa.campaign_id NOTNULL AND tsa.engine = 'google' THEN (
					...
					ELSE NULL
					END) as adgroup_name,
					 tsa.creation_platform as creation_platform,
					 tsa.cntry_first_address
				FROM tmp_sem_attribution tsa
				WHERE date_trunc('day', tsa.first_successful_payment_transferred) > current_date - 10
				AND date_trunc('day', tsa.first_successful_payment_transferred) < current_date;
...
WITH new_users AS (
	...
	SELECT
		...
		COALESCE(new_users.mnu, new_users.mnu) as new_users,
